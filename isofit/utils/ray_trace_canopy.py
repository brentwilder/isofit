...
def ray_trace(i, j, dem_arr, sza, saa, pix_size):
    ...
    try:
        interp_func = interpolate.interp1d(np.arange(len(zi)), zi, kind='cubic', fill_value="extrapolate")
    except:
        return [i,j,0]
    
    fine_zi = interp_func(np.linspace(0, len(zi)-1, 10*len(zi)))  # Upsample by a factor of 10
    fine_h = np.interp(np.linspace(0, len(zi)-1, 10*len(zi)), np.arange(len(zi)), h)

    intersections = fine_zi - fine_h


    idx_intersections = np.where(np.diff(np.sign(intersections)))[0]  # Change in sign =intersection
...
if __name__ == '__main__':
    # inputs
    n_cpu=12
    output = '/Users/bawilder/Documents/SNOW/LIDAR/Alaska23/dsm_0.5m_small_raytracing.tif'
