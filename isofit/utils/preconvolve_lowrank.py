...
def load_eco_data(path, WAVE, FWHM):
    ...
    if "soil" in path:
        interp_type = "nearest"
    rho_i = interpolate.interp1d(
        d["Wavelength"], d["Reflectance"], kind=interp_type, fill_value="extrapolate"
    )
...
def collect_spectra_by_type(folder, types, WAVE, FWHM):
    ...
    for fname in os.listdir(folder):
        if fname.lower().endswith(".spectrum.txt"):
            ...
            if (
                "vegetation" in lower_fname
                and "vswir" in lower_fname
                and "tree" in lower_fname
            ):
                t = "vegetation"
    ...
    for t in types:
        if grouped[t]:
            df = pd.DataFrame(
                np.column_stack(grouped[t]),
                columns=[f"rho_{i}" for i in range(len(grouped[t]))],
            )
