...
class IceSurface(MultiComponentSurface):
    ...
    def __init__(self, full_config: Config):
        ...
        # entire state vector
        self.statevec_names = (['cosi', # topography
                                'Grain_radius','Algae', 'Dust',  'Liquid_water'# ice properties (grain is actually bubble radius)
                                ])
        self.scale = [1., 
                      1., 1., 1.,1.,
                      ]
        ...
        self.bounds = np.array([
            ...
            [0.0, 1.],           # dust concentration      #TODO DEBUG
    ...
    def calc_rfl(self, x_surface, geom, L_down_dir=None, L_down_dif=None):
        ...
        #disort_raa = 180 - raa
        ice_dir_dir = self.g_brdf(np.array([np.degrees(np.arccos(x_surface[0])), 
                                            x_surface[1],x_surface[2], x_surface[3], x_surface[4], 4.5]))
        
        ice_dif_dir = self.g_hdrf(np.array([np.degrees(np.arccos(x_surface[0])), 
                                            x_surface[1],x_surface[2], x_surface[3], x_surface[4], 4.5]))
