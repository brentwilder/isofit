...
class SnowSurface(MultiComponentSurface):
    ...
    def __init__(self, full_config: Config):
        ...
        self.statevec_names = [
            ...
            "Liquid_water",
            "Dust",
            "Algae",  # snow properties (4)
            "z_snow",
            "z_pv",
            "z_npv",
            "z_soil",  # pixel surface fractional covers (4)
            "veg_rank",
            "npv_rank",
            "soil_rank",  # endmember parameters (3)
        ]
        self.scale = [
            1.0,
            1.0,
        ...
        self.bounds = np.array(
            [
                [-1.0, 1.0],  # Sine aspect
                [-1.0, 1.0],  # Cosine aspect
                [30.0, 1500.0],  # grain radius
                [0.0, 25.0],  # liquid water fraction
                [0.0, 4000.0],  # dust concentration
    ...
    def calc_rfl(self, x_surface, geom, L_down_dir=None, L_down_dif=None):
        ...
        rho_dir_dir = (
            snow_dir_dir * f[0] + f[1] * rho_pv + f[2] * rho_npv + f[3] * rho_soil
        )
        rho_dif_dir = (
            snow_dif_dir * f[0] + f[1] * rho_pv + f[2] * rho_npv + f[3] * rho_soil
        )
    ...
    def calc_new_angles(self, x_surface, sza, vza, saa, vaa, slope, geom):
        ...
        cosi = np.sin(np.radians(sza)) * np.sin(np.radians(slope)) * np.cos(
            np.radians(saa) - aspect
        ) + np.cos(np.radians(sza)) * np.cos(np.radians(slope))

        cosv = np.sin(np.radians(vza)) * np.sin(np.radians(slope)) * np.cos(
            np.radians(vaa) - aspect
    ...
    def drdn_dsurface(self, x_surface, geom, L_down_dir=None, L_down_dif=None):
        ...
        svf = geom.svf
        cosi, _ = self.calc_new_angles(
            x_surface,
            geom.solar_zenith,
            geom.observer_zenith,
            geom.solar_azimuth,
            geom.observer_azimuth,
            geom.slope,
        )
        ...
        for x_surface_perturb in x_surfaces_perturb:
            ...
            # get new angles
            cosi_perturb, _ = self.calc_new_angles(
                x_surface_perturb,
                geom.solar_zenith,
                geom.observer_zenith,
                geom.solar_azimuth,
